...
function HoverGradientNavBar({ locale, bannerVisible }: HoverGradientNavBarProps): React.JSX.Element {
  ...
  return (
    <div className={`hidden md:block fixed top-0 left-0 w-full md:left-1/2 md:-translate-x-1/2 z-50 ${bannerVisible ? 'top-12 md:top-16' : 'top-0 md:top-4'}`}>
      <motion.nav
        className="w-full md:w-fit mx-auto px-2 py-2 rounded-none md:rounded-3xl
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-48 bg-background/90 backdrop-blur-lg border border-border/80 shadow-lg">
                ...
                <DropdownMenuItem onClick={toggleTheme}>
                  ...
                  <span>
                    {theme === 'dark' ? (
                      <FormattedMessage id="common.theme-switcher.dark" defaultMessage="Switch to light mode" />
                    ) : (
                      <FormattedMessage id="common.theme-switcher.light" defaultMessage="Switch to dark mode" />
                    )}
                  </span>
                </DropdownMenuItem>
