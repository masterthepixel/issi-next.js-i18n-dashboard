...
import { BreadcrumbDevHelper } from '@/utils/breadcrumbDevHelper';
 ...
 * - Development mode with missing translation detection
...
export const UniversalBreadcrumb = ({ 
  ...
  useEffect(() => {
    ...
    const generateBreadcrumb = () => {
      ...
      // Development helpers
      if (enableDevMode) {
        // Log auto-generated content for developers
        const autoGeneratedItems = items.filter(item => item.isAutoGenerated);
        if (autoGeneratedItems.length > 0) {
          console.group('🤖 Auto-generated breadcrumb content');
          console.log('Items:', autoGeneratedItems.map(item => ({
            label: item.label,
            translationKey: item.translationKey
          })));
          console.log('💡 Consider adding proper translations for better UX');
          console.groupEnd();
        ...
        if (pathname) {
          ...
          const validation = BreadcrumbDevHelper.validateBreadcrumbConfig(pathname, lang, messages);
          if (!validation.isValid) {
            console.group('⚠️ Breadcrumb Configuration Issues');
    ...
    if (enableDevMode) {
      BreadcrumbDevHelper.measureBreadcrumbPerformance(generateBreadcrumb);
    } else {
      generateBreadcrumb();
    }

  }, [pathname, lang, messages, enableAutoGeneration, enableDevMode]);
